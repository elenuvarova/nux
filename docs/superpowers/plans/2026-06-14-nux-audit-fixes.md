...
- [x] G2 (font subset) — DONE + preview-verified (no tofu). Self-hosted subset `@font-face` (`src/styles/fonts.css`) replaces the `@fontsource` imports: **138KB → 73KB eager fonts (−47%)** on the render-blocking path. Fraunces instanced to its only weight (600) and Newsreader to its only weight (400) — the variable axis was the real size driver, not glyphs; Inter stays variable (400–700). Subset to the exact glyphs the source renders (scanned: Basic Latin + Latin-1 ©·×éëö + — ' " … + arrows + ★✕✦), verified live: ë (Noël), ·, ★, —, ✦ all render in the right family/weight. Vite hashes the woff2 (immutable cache intact). Audit's "290→110" estimate was off — latin-ext never loaded.
